...
class EnterYourInfos(FlaskForm):
    ...
    aType = IntegerField("What kind of article? World(1) Tech(2) Social(3) Business(4) Entertainement(5) Lifestyle(6) ", validators=[DataRequired(), NumberRange(min=1, max=6)])
...
def show_user_the_form():
    ...
    if request.method == "POST" and myform.validate_on_submit():
        model_logistic = joblib.load("./xgbmodel2")
        prediction2 = str(Prediction(myform.title.data, myform.text.data, myform.aType.data - 1))
